...
- **Node.js**: 18+
- **npm**: 9+
- **磁盘空间**: 约 2GB（其中 sentence-transformers 向量模型约 400MB，ChromaDB 数据视知识库规模而定）
...
pip install -r requirements.txt         # 安装依赖（含 ChromaDB、sentence-transformers 等）
```

> ⚠️ `sentence-transformers` 首次运行时会自动下载向量模型（约 400MB），请确保网络畅通。
...
```bash
python app.py                            # 首次运行自动创建数据库表和向量集合
```
...
将课程文档放入 `backend/data/` 目录，然后运行导入脚本：

```bash
cd backend
python scripts/import_knowledge.py      # 批量导入知识库
```
...
2. **登录系统**: 输入用户名密码登录
3. **智能问答**: 在"对话"标签页中输入计算思维相关问题
4. **代码分析**: 切换到"代码分析"标签页，粘贴 Python 代码进行分析
5. **知识库管理**: 切换到"知识库"标签页，上传课程资料
6. **查看学习报告**: 切换到"学习分析"标签页，查看学习统计与薄弱点
